---
title: "Supplementary Text S2: Assessing orthogroup inference in public databases"
...
vignette: |
  %\VignetteIndexEntry{Supplementary Text S2: Assessing orthogroup inference in public databases}
...
- OrthoDB [@zdobnov2021orthodb]
- eggNOG [@huerta2019eggnog]
...
## Qiao *et al.*, 2019. Genome Biology

Orthogroups from @qiao2019gene are available in [this FigShare repository](https://figshare.com/articles/dataset/Gene_duplication_and_evolution_in_recurring_polyploidization-diploidization_cycles_in_plants/7264667/1). 
The orthogroups information are in the *Orthogroups.csv.zip* file.

```{r qiao_et_al}
# Download file and unzip it
download.file(
    url = "https://figshare.com/ndownloader/files/13382270",
    destfile = file.path(tempdir(), "Orthogroups.zip")
)
unzip(
    zipfile = file.path(tempdir(), "Orthogroups.zip"),
    exdir = tempdir()
)

# Get orthogroups
## This file is an old OrthoFinder output without "Orthogroup" in the first row
## Let's add it manually, so it can be parsed with read_orthogroups
og_file <- file.path(tempdir(), "Orthogroups.csv")
l <- readLines(og_file)
l[1] <- paste0("Orthogroup", l[1])
writeLines(l, con = og_file)

## Read and parse file
fam_qiao <- cogeqc::read_orthogroups(og_file) %>%
    mutate(Species = str_replace_all(Species, "\\.pep.*", "")) %>%
    filter(Species == "Ath") %>%
    mutate(Gene = str_replace_all(Gene, "\\.[0-9]$", "")) %>%
    as.data.frame()

# Combining everything and calculating homogeneity scores
fam_df_qiao <- merge(fam_qiao, ath_interpro)
H_qiao <- H_summary(fam_df_qiao)
```
...
H_combined <- bind_rows(
    ...
    H_qiao$H %>% mutate(Source = "Qiao et al."),
...
```{r load_combined, echo = FALSE, eval = TRUE}
...
```{r compare_summary_stats, eval = TRUE}
...
db_wilcox[db_wilcox$padj_interpretation != "ns", ]
```
...
2. HOGENOM orthogroups have higher scores than OrthoDB, but lower than 
Qiao *et al.* and PLAZA.

3. Orthogroups scores from Qiao *et al* are higher than all other sources.

4. Among the databases (excluding Qiao *et al*), PLAZA orthogroup scores are
higher than every other database.
...
```{r visualize_distros, eval = TRUE, warning = FALSE, message = FALSE, dpi = 200}
# Comparisons to be made
comps <- list(
    c("Qiao et al.", "PLAZA"),
    c("Qiao et al.", "HOGENOM"),
    c("Qiao et al.", "OrthoDB"),
    c("Qiao et al.", "eggNOG"),
    c("HOGENOM", "eggNOG")
)
...
H_combined$Source <- factor(
    H_combined$Source, levels = rev(c(
        "Qiao et al.", "PLAZA", "HOGENOM", "OrthoDB", "eggNOG"
    ))
)
...
distros <- ggviolin(
    ...
    orientation = "horiz", trim = TRUE, add = "boxplot", 
