---
title: "Supplementary Text S3: Assessing orthogroup inference for Brassicaceae genomes"
shorttitle: "cogeqc: an R/Bioconductor package for quality checks in comparative genomics"
author:
...
vignette: |
  %\VignetteIndexEntry{Supplementary Text S3: Assessing orthogroup inference for Brassicaceae genomes}
...
```{r load_fig, echo = FALSE, eval = TRUE, dpi = 200}
...
```{r show_plot2, echo = FALSE, eval = TRUE, fig.width = 10, fig.height = 5}
...
```{r og_distros_compare, eval = TRUE}
# Relevel 'Mode' factor
...
# Comparing to the default mode (default DIAMOND, mcl = 1.5)
comp_global_default <- compare(
    og_assessment, "Median_score ~ Mode", "default_1_5"
)
comp_global_default
```
...
```{r og_distros_viz, eval = TRUE, fig.width=8, fig.height=6}
# Visualize
global_comps <- list(
...
save(
    p_distros, compress = "xz",
    file = here("products", "plots", "h_distros_global.rda")
)
```

## The effect of species choice

Here, we will compare how scores vary for each species to check if the species
choice can influence the distribution of orthogroup homogeneity scores.

```{r effect_of_species_choice, eval = TRUE}
# Convert data frame from wide to long to include species information
og_byspecies <- og_assessment %>% 
    filter(Mode == "default_1_5") %>%
    dplyr::select(c(1:9)) %>%
    pivot_longer(
        cols = !Orthogroups,
        names_to = "species",
        values_to = "homogeneity"
    ) %>%
    mutate(
        species = str_replace_all(species, "_.*", "")
    )

# Compare each species to the mean
comp_species <- compare(og_byspecies, "homogeneity ~ species", "Mean")
comp_species
```

The Wilcoxon tests show that homogeneity scores for *B. carinata* are greater
than the mean score, while the mean score is greater than all other species
scores. However, the effect sizes are small. Thus, we conclude that the species 
choice should not be a concern when assessing homogeneity scores.

Let's visualize the distributions and highlighting significant differences
with effect sizes > 0.1.

```{r viz_species_H, eval = TRUE, fig.width = 8, fig.height = 6}
# Plot
comp_species <- list(c("Mean", "Aarabicum"))

p_species <- og_byspecies %>%
    mutate(homogeneity = homogeneity / max(homogeneity, na.rm = TRUE)) %>%
    ggviolin(
        ., y = "homogeneity", x = "species", 
        orientation = "horiz", trim = TRUE,
        add = c("boxplot", "mean"), 
        fill = "species", add.params = list(fill = "white"), palette = "nejm"
    ) +
    stat_compare_means(
        comparisons = comp_species, label = "p.signif",
        method = "wilcox.test"
    ) +
    theme(legend.position = "none") +
    labs(y = "Homogeneity scores", x = "",
         title = "Distribution of mean homogeneity scores per species") +
    theme(plot.subtitle = ggtext::element_markdown())

p_species
```

```{r save_species_H, echo = FALSE}
save(
    p_species, compress = "xz",
    file = here("products", "plots", "h_distros_species.rda")
)
```
...
```{r viz_og_mcl, eval = TRUE, fig.width = 8, fig.height = 5, dpi = 200}
...
save(
    p_mcl, compress = "xz",
    file = here("products", "plots", "h_distros_mcl.rda")
)
```
...
```{r og_diamond, eval = TRUE, fig.width = 8, fig.height = 6, dpi = 200}
# Plot
p_diamond <- og_modes %>%
...
save(
    p_diamond, compress = "xz",
    file = here("products", "plots", "h_distros_diamond.rda")
)
```
...
```{r plot_distros_with_groups, eval = TRUE}
...
```{r plot_treeplot, eval=TRUE, dpi=200, fig.height=9, fig.width=20}
# Tree plot
...
```{r plot_dotplot, fig.height=10, fig.width=18, eval = TRUE,dpi=200}
# Dot plot
...
```{r plot_og_length_h, eval = TRUE, fig.width = 8, fig.height = 5}
p_association <- og_length_and_scores %>%
    mutate(
        logH = log10(Median_H),
        logLength = log10(median_length)
    ) %>%
    ggscatter(
        ...
        cor.coef = TRUE, 
        cor.coeff.args = list(
            method = "spearman", label.x = 1.7, label.y = -1, label.sep = "\n"
        )
    ) +
    labs(
        title = "Relationship between OG homogeneity score and gene length",
...
There is a significant, negative association between homogeneity and OG gene
length, but the strength of association is small ($R^2 = -0.2$).
Although weak, this negative relationship shows that OrthoFinder can
reduce the gene length bias, but not completely. 
